...
import com.app.common.help.SystemBarTintManager;
...
/**
 * Created by QinQin on 2017/11/7.
 */
...
public class StatusBarUtil {
    ...
    public static void setDrawable(Activity activity, @DrawableRes int drawableId) {
        ...
        setResourceDrawable(activity, drawableId);
    }
     ...
     * @param drawable
     */
    public static void setResourceDrawable(Activity activity, int drawable) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            // 去掉半透明
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        ...
        if (fakeStatusBarView != null) {
            ...
            fakeStatusBarView.setBackgroundResource(drawable);
     ...
     * @return
     */
    private static View createStatusBarDrawableView(Activity activity, int drawable) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity) + 5);
        statusBarView.setLayoutParams(params);
